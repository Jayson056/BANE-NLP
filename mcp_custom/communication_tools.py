...
@mcp_custom_tool(
    ...
    description="Send a professional BANE-branded HTML email."
)
...
def send_telegram_message(text: str, user_id: str = None):
    ...
    chat_id = user_id if user_id else getattr(config, "ALLOWED_TELEGRAM_USERS", [])
...
@mcp_custom_tool(name="communication_tools.send_telegram_file", description="Send a file/document to Telegram.")
def send_telegram_file(file_path: str, user_id: str = None, caption: str = None):
    token = getattr(config, "TELEGRAM_TOKEN", None)
    chat_id = user_id if user_id else getattr(config, "ALLOWED_TELEGRAM_USERS", [])
    ...
    try:
        ...
        return f"✅ File sent to Telegram."
    except Exception as e:
        return f"❌ Failed: {str(e)}"
