...
class MCPRegistry:
    ...
    def _load_all_tools(self):
        """Dynamically load all functions decorated with @mcp_custom_tool in mcp/ modules."""
        import mcp_custom
        prefix = mcp_custom.__name__ + "."
        for importer, modname, ispkg in pkgutil.iter_modules(mcp_custom.__path__, prefix):
            try:
                module = importlib.import_module(modname)
                for name, obj in inspect.getmembers(module, inspect.isfunction):
                    if getattr(obj, "_is_mcp_custom_tool", False):
                        tool_name = getattr(obj, "_mcp_custom_tool_name", name)
    ...
    def _load_dynamic_tools(self):
        ...
        for filename in os.listdir(self._dynamic_tools_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                modname = f"mcp_custom.dynamic.{filename[:-3]}"
                try:
    ...
    def register_dynamic_tool(self, tool_name: str, code: str, description: str = "") -> str:
        ...
        The tool must use the @mcp_custom_tool decorator.
        ...
        try:
            ...
            modname = f"mcp_custom.dynamic.{safe_name}"
            if modname in sys.modules:
                module = importlib.reload(sys.modules[modname])
            else:
                module = importlib.import_module(modname)
            
            # Register any @mcp_custom_tool decorated functions from the module
            registered = []
            for name, obj in inspect.getmembers(module, inspect.isfunction):
                if getattr(obj, "_is_mcp_custom_tool", False):
                    tname = getattr(obj, "_mcp_custom_tool_name", name)
            ...
            if registered:
                ...
                return f"⚠️ File {safe_name}.py was saved but no @mcp_custom_tool decorated functions were found."
    ...
    def get_tool_list_summary(self) -> str:
        ...
        for name, func in sorted(self.tools.items()):
            desc = getattr(func, "_mcp_custom_tool_desc", "")
...
'{"call_tool":"meta_tools.create_tool","args":{"tool_name":"...","description":"...","code":"from mcp_custom.mcp_registry import mcp_custom_tool\\n@mcp_custom_tool(name=..."}},',
            "",
            "Tools:",
        ]
