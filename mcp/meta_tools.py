...
@mcp_tool(
    ...
    description="Create and register a new MCP tool at runtime (no restart needed). Args: {'tool_name': 'category.function_name', 'description': 'what it does', 'code': 'full python source code with @mcp_tool decorator'}"
)
def create_tool(tool_name: str, description: str = "", code: str = "") -> str:
    """
    Create a new MCP tool dynamically. The code must include the @mcp_tool decorator.
    
    Example code format:
        from mcp.mcp_registry import mcp_tool
        
        @mcp_tool(name="my_tools.greet", description="Say hello")
    ...
    Args:
        ...
        code: Full Python source code including imports and @mcp_tool decorator
    """
    if not tool_name:
        return "❌ Error: 'tool_name' is required."
    if not code:
        return "❌ Error: 'code' is required. Must include a function with @mcp_tool decorator."
    ...
    if "from mcp.mcp_registry import mcp_tool" not in code:
