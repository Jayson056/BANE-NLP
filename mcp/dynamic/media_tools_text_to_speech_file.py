...
@mcp_tool(name="media_tools.text_to_speech_file", description="Converts text string to an audio file using edge-tts. Args: {'text': 'the exact text to say', 'output_path': 'path to output file (.ogg)', 'voice': '(optional) en-US-GuyNeural (male) or en-US-JennyNeural (female)'}")
async def text_to_speech_file(text: str, output_path: str, voice: str = "en-US-GuyNeural") -> str:
    try:
        ...
        if output_path.lower().endswith('.ogg'):
            # Convert to strictly formatted Libopus OGG for telegram asynchronously to prevent event loop blocking
            proc = await asyncio.create_subprocess_exec(
                'ffmpeg', '-y', '-i', temp_mp3, '-c:a', 'libopus', '-b:a', '48k', output_path,
